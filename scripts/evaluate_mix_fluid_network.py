...
def evaluate_tf(model, val_dataset, frame_skip, fluid_errors=None, scale=1):
    ...
    for data in val_dataset:
        ...
        if data['frame_id0'][0] % skip == 3:
            # 确保收集到足够的帧数
            if len(frames) >= 3 and all(f['scene_id0'][0] == frames[0]['scene_id0'][0] for f in frames):
                ...
                # 根据模型类型调用不同的预测方式
                if has_phases and phase_fractions is not None:
                    # 多相流体模型预测
                    cd = frames[0].get('cd', 0.5)
                    cf = frames[0].get('cf', 0.5)
