...
from models.default_tf_mix_separate_pos_phase_v10 import MultiPhaseParticleNetwork
    ...
    def train_step(model_instance, optimizer_instance, current_batch):
        with tf.GradientTape() as tape:
            ...
            for i in range(train_params.batch_size):
                ...
                cd_val = tf.cast(current_batch['cd'][i], tf.float32)
                cf_val = tf.cast(current_batch['cf'][i], tf.float32)
