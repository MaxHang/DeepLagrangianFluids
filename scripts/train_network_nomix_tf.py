...
from datasets.dataset_reader_h5_nomix import read_data_train, read_data_val
import numpy as np
import argparse
import yaml
    ...
    if gpu_id is not None:
        ...
        if gpus:
            try:
                ...
                print(e)
    from models.default_tf_nomix import MyParticleNetwork
...
def main():
    ...
    def train(model, batch):
        with tf.GradientTape() as tape:
            ...
            debug_print("batch_size_real: ", len(batch['pos0']))
            for batch_i in range(batch_size):
                # 第一帧预测
                inputs = ([
                    batch['pos0'][batch_i], batch['vel0'][batch_i], batch['density'][batch_i], None,
                ...
                inputs = (pr_pos1, pr_vel1, batch['density'][batch_i], None, batch['box'][batch_i],
    ...
    while trainer.keep_training(checkpoint.step,
        ...
        batch_tf = {}
        for k in ('pos0', 'vel0', 'pos1', 'pos2', 'box', 'box_normals', 'density'):
            if k in batch:
