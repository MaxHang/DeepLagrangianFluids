...
from models.default_tf_mix_separate_pos_phase_v5 import MultiPhaseParticleNetwork
...
# def save_complete_config(cfg, train_dir, train_params, model_config):
...
def save_complete_config(cfg, train_dir, train_params, model_config, 
                        loss_weights, vf_loss_type, use_mass_conservation, 
                        mass_conservation_weight, lr_boundaries, lr_values_actual, 
                        optimizer_epsilon, num_workers, loss_scaling_factor):
    """保存完整的配置文件到训练目录，确保与实际使用的参数一致"""
    # 创建完整的配置字典 - 使用实际运行时的参数
    complete_cfg = {
        'dataset_dir': cfg.get('dataset_dir', ''),
        'model': model_config,
        ...
        'optimizer': {
            ...
            'lr_values_actual': lr_values_actual,  # 保存实际的学习率值
            'epsilon': optimizer_epsilon,
        },
        'loss_weights': {
            'pos': loss_weights.get('pos', 1.0),
            'vol': loss_weights.get('vol', 1.0),
            'gamma': loss_weights.get('gamma', 0.5),
            'use_mass_conservation': use_mass_conservation,  # 实际使用的值
            'mass_conservation': mass_conservation_weight,   # 实际使用的权重
        },
        'loss_vf_type': vf_loss_type,
    ...
    print(f"Complete training config saved to: {config_save_path}")
    print(f"  - Mass conservation: {use_mass_conservation} (weight: {mass_conservation_weight})")
    print(f"  - Learning rate boundaries: {lr_boundaries}")
    print(f"  - Actual LR values: {lr_values_actual}")
    return config_save_path
...
def main():
    ...
    use_zero_sum_game = cfg['model'].get('use_zero_sum_game', False)
    ...
    save_complete_config(
        cfg, train_dir, train_params, model.init_params,
        loss_weights, vf_loss_type, use_mass_conservation, 
        mass_conservation_weight, lr_boundaries, lr_values_actual,
        optimizer_epsilon, num_workers, loss_scaling_factor
    )
