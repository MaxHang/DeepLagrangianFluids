...
YAML_FILE="scripts_nomix/nomix-fluid-no_mcconv.yaml"
DATE=$(date +"%Y%m%d_%H%M%S")  # 获取当前日期和时间，格式为 YYYYMMDD_HHMMSS
LOG_FILE="scripts_nomix/run_nomix_no_mmcconv_${DATE}.log"  # 定义日志文件名，包含日期和时间
SCENE="scripts_nomix/run_nomix_valid.json"
OUTPUT="/workspace/xyh_synology/graduate/run_network/nomix_fluid/ply_${DATE}"
NUMSTEPS=400
GPU_ID="0"  # 默认使用GPU 0
WEIGHTS="/workspace/xyh_synology/graduate/weights/nomix-fluid/ablation-no_mcconv/train_network_nomix_tf_nomix-fluid-no_mcconv_2025_04_24/checkpoints/ckpt-5000.index"
