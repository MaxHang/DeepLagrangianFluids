...
YAML_FILE="/workspace/xyh_synology/graduate/weights/nomix-fluid/density_ratio/20260331090333/training_config.yaml"
DATE=$(date +"%Y%m%d_%H%M%S")  # 获取当前日期和时间，格式为 YYYYMMDD_HHMMSS
LOG_FILE="scripts_nomix/log_eval/eval_nomix_dens_ratio_${DATE}.log"  # 定义日志文件名，包含日期和时间
SCENE="scripts_nomix/run_nomix_valid.json"
OUTPUT="/workspace/xyh_synology/graduate/run_network/nomix_fluid/ply_${DATE}"
NUMSTEPS=400
GPU_ID="0"  # 默认使用GPU 0
WEIGHTS="/workspace/xyh_synology/graduate/weights/nomix-fluid/density_ratio/20260331090333/model_weights_2026_04_01.h5"
