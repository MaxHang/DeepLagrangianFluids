...
核心设计：
  1. DeepSet 编码相集合（vf + density）→ 置换不变的全局相嵌入，不依赖相数。
  2. VF 逐相预测（权重共享 MLP）：
       输入 = [粒子共享特征 | 该相的 vf + log_density]
       对所有相用同一 MLP，因此训练时用 2 相，推理时可直接泛化到任意相数。
  3. cd/cf 条件编码：MLP 将 (cd, cf) 编码为向量，拼接到粒子特征，
...
class MultiPhaseParticleNetwork(tf.keras.Model):
    ...
    位置预测：ContinuousConv 主干 → 位置修正头
    VF  预测：共享特征 + 逐相特征 → 权重共享 MLP → delta_vf → 守恒归一化
    ...
    def call(self,
        ...
        # 2. 相特征归一化（同时服务于 DeepSet 编码和 VF 逐相预测器）
    ...
    def _predict_next_vf(self,
        ...
        vf_next = tf.keras.activations.relu(current_vf + 0.1 * delta_vf)
