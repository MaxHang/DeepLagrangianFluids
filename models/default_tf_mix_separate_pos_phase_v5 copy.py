...
"""
创建日期: 2025-10-20
文件名: default_tf_mix_separate_pos_phase_v5.py
版本: V5

描述: 
    本文件定义了一个用于数据驱动多相流体模拟的深度学习网络模型（第5版）。
    该模型基于拉格朗日粒子法，并使用连续卷积作为核心的空间信息聚合算子。
    
V5 版本的核心改进（基于V4）:
    1. 改进的条件参数集成：
       - 支持交叉注意力机制：让粒子根据自身状态动态感知全局 cd/cf
       - 支持 FiLM 条件归一化：在主干网络中逐层调制特征
       - 支持物理增强特征：显式编码 cd/cf 的物理效应
    
    2. 灵活的组件控制：
       - use_cd_cf_attention: 是否使用交叉注意力
       - use_cd_cf_film: 是否使用 FiLM 调制
       - use_cd_cf_physics: 是否使用物理增强特征
       - 可以任意组合以适应不同场景
...
class MultiPhaseParticleNetwork(tf.keras.Model):
    ...
    - 多尺度物理属性的鲁棒特征编码
    - 灵活的条件参数集成（注意力 + FiLM + 物理增强）
    - 端到端的粒子位置和相分布联合预测

    # 示例1：使用所有组件（默认）
    model_full = MultiPhaseParticleNetwork(
        max_num_phases=3,
        use_cd_cf_attention=True,   # 交叉注意力
        use_cd_cf_film=True,         # FiLM 调制
        use_cd_cf_physics=True       # 物理增强
    )

    # 示例2：只使用 FiLM
    model_film_only = MultiPhaseParticleNetwork(
        max_num_phases=3,
        use_cd_cf_attention=False,
        use_cd_cf_film=True,
        use_cd_cf_physics=False
    )

    # 示例3：只使用物理增强
    model_physics_only = MultiPhaseParticleNetwork(
        max_num_phases=3,
        use_cd_cf_attention=False,
        use_cd_cf_film=False,
        use_cd_cf_physics=True
    )

    # 示例4：关闭所有 cd/cf 集成（退化到 V4）
    model_no_cdcf = MultiPhaseParticleNetwork(
        max_num_phases=3,
        cd_cf_as_input=False
    )
    """
    ...
    def __init__(self,
                 ...
                 use_cd_cf_attention: bool = True,      # 是否使用交叉注意力
                 use_cd_cf_film: bool = True,           # 是否使用 FiLM 调制
                 use_cd_cf_physics: bool = True,        # 是否使用物理增强特征
                 cd_cf_attention_heads: int = 4,        # 注意力头数
                 cd_cf_embedding_dim: int = 64,         # cd/cf 编码维度
                 # --- 物理与模拟参数 ---
        ...
        Args:
            ...
            cd_cf_as_input: 是否使用 cd/cf 作为条件输入（总开关）。
            use_cd_cf_attention: 是否使用交叉注意力机制让粒子动态感知 cd/cf。
                优势：每个粒子根据自身状态得到不同的 cd/cf "视图"
            use_cd_cf_film: 是否使用 FiLM 在主干网络中逐层调制特征。
                优势：cd/cf 影响深入到每一层，全局控制力强
            use_cd_cf_physics: 是否添加物理增强特征（速度-cd、扩散-cf）。
                优势：显式编码 cd/cf 的物理含义，帮助网络理解
            cd_cf_attention_heads: 交叉注意力的头数（仅当 use_cd_cf_attention=True 时有效）。
            cd_cf_embedding_dim: cd/cf 编码的维度。
        ...
        self.use_cd_cf_attention = use_cd_cf_attention
        self.use_cd_cf_film = use_cd_cf_film
        self.use_cd_cf_physics = use_cd_cf_physics
        self.cd_cf_attention_heads = cd_cf_attention_heads
        ...
        # ========== 打印初始化信息 ==========
        print(f"\n{'='*80}")
        print(f"[Model V5] Initializing MultiPhaseParticleNetwork")
        print(f"{'='*80}")
        ...
        print(f"\n  [V5 Conditional Integration]")
        print(f"  cd/cf as input          : {self.cd_cf_as_input}")
        if self.cd_cf_as_input:
            print(f"    - Cross Attention     : {self.use_cd_cf_attention} (heads={self.cd_cf_attention_heads if self.use_cd_cf_attention else 'N/A'})")
            print(f"    - FiLM Modulation     : {self.use_cd_cf_film}")
            print(f"    - Physics Features    : {self.use_cd_cf_physics}")
        print(f"{'='*80}\n")
        ...
        if self.cd_cf_as_input:
            ...
            self.cd_cf_encoder = tf.keras.Sequential([
                ...
                tf.keras.layers.Dense(self.cd_cf_embedding_dim, activation='relu', name='cd_cf_enc2'),
            ], name='cd_cf_encoder')

            # 构建 cd_cf_encoder 的权重
            dummy_cdcf = tf.zeros((1, 2), dtype=tf.float32)
            _ = self.cd_cf_encoder(dummy_cdcf)
            
            # 2. 交叉注意力机制
            if self.use_cd_cf_attention:
                self.cd_cf_attention = tf.keras.layers.MultiHeadAttention(
                    num_heads=self.cd_cf_attention_heads,
                    key_dim=self.cd_cf_embedding_dim // self.cd_cf_attention_heads,
                    name='cd_cf_cross_attention'
                )
            
            # 3. FiLM 调制层
            if self.use_cd_cf_film:
                self.cd_cf_film_layers = []
                for i, ch in enumerate(self.shared_feature_channels):
                    # 先创建网络对象
                    gamma_net = tf.keras.Sequential([
                        tf.keras.layers.Dense(32, activation='relu', name=f'gamma_hidden_{i}'),
                        tf.keras.layers.Dense(ch, name=f'gamma_{i}')
                    ], name=f'gamma_net_{i}')
                    
                    beta_net = tf.keras.Sequential([
                        tf.keras.layers.Dense(32, activation='relu', name=f'beta_hidden_{i}'),
                        tf.keras.layers.Dense(ch, name=f'beta_{i}')
                    ], name=f'beta_net_{i}')

                    # 立即构建权重
                    dummy_embedding = tf.zeros((1, self.cd_cf_embedding_dim), dtype=tf.float32)
                    _ = gamma_net(dummy_embedding)
                    _ = beta_net(dummy_embedding)
                    
                    self.cd_cf_film_layers.append({
                        'gamma_net': gamma_net,
                        'beta_net': beta_net
                    })
        ...
        self.vf_conv_heads = []  # 卷积层列表
        self.vf_dense_heads = []  # 全连接层列表
        self.vf_output_dims = []  # 输出维度列表（普通 Python list，不会被追踪）
        
        if self.max_num_phases > 1:
            print(f"[Model V5] Creating dynamic VF prediction heads...")
            for num_phases in range(2, self.max_num_phases + 1):
                ...
                # 创建该相数对应的卷积和全连接层
                self.vf_conv_heads.append(Conv(name=f"vf_conv_p{num_phases}", filters=output_dim))
                self.vf_dense_heads.append(tf.keras.layers.Dense(units=output_dim, name=f"vf_dense_p{num_phases}"))
                self.vf_output_dims.append(output_dim)
            print(f"[Model V5] Dynamic VF heads created for {len(self.vf_conv_heads)} configurations.\n")
    ...
    def compute_shared_features(self, 
        ...
        V5 条件参数集成流程：
            1. 编码：将 (cd, cf) 编码为高维表示
            2. 注意力（可选）：让粒子根据自身状态查询 cd/cf
            3. 物理增强（可选）：显式计算 cd/cf 的物理效应
            4. 特征拼接：组合所有特征
            5. FiLM 调制（可选）：在主干网络中逐层调制
        """
        ...
        # ========== 3. 条件参数处理（V5 核心改进）==========
        if self.cd_cf_as_input:
            cd_scalar = kwargs.get('cd')
            cf_scalar = kwargs.get('cf')

            # debug_print(f"cd: {cd_scalar}, cf: {cf_scalar}")
            
            # 步骤1: 编码 cd/cf 为高维表示
            global_condition = tf.convert_to_tensor([[cd_scalar, cf_scalar]], dtype=tf.float32)
            global_cd_cf_embedding = self.cd_cf_encoder(global_condition)  # [1, cd_cf_embedding_dim]
            
            # 步骤2: 交叉注意力（可选）
            if self.use_cd_cf_attention:
                # 让每个粒子根据自身相嵌入"查询"全局 cd/cf
                query = tf.expand_dims(particle_phase_embedding, axis=0)  # [1, N, 64]
                key_value = tf.expand_dims(global_cd_cf_embedding, axis=0)  # [1, 1, cd_cf_embedding_dim]
                
                attended_cd_cf = self.cd_cf_attention(
                    query=query, 
                    value=key_value, 
                    key=key_value
                )  # [1, N, cd_cf_embedding_dim]
                attended_cd_cf = tf.squeeze(attended_cd_cf, axis=0)  # [N, cd_cf_embedding_dim]
                
                fluid_feats_list.append(attended_cd_cf)
            
            # 步骤3: 物理增强特征（可选）
            if self.use_cd_cf_physics:
                # 3.1 cd 影响速度（漂移特性）
                # cd = 0: 自由漂移，速度影响大
                # cd = 1: 无漂移，速度影响小
                # velocity_magnitude = tf.norm(vel, axis=-1, keepdims=True)  # [N, 1]
                # 添加 epsilon 避免零速度时梯度不稳定
                velocity_magnitude = tf.sqrt(tf.reduce_sum(tf.square(vel), axis=-1, keepdims=True) + 1e-8) # [N, 1]
                cd_velocity_coupling = velocity_magnitude * (1.0 - cd_scalar)  # [N, 1]
                
                # 3.2 cf 影响扩散（与 VF 分散程度相关）
                # cf 越大，扩散越强
                vf_variance = tf.math.reduce_std(phase_fractions, axis=-1, keepdims=True)  # [N, 1]
                vf_variance_safe = tf.maximum(vf_variance, 1e-6)  # 避免零方差导致 NaN
                cf_diffusion_coupling = vf_variance_safe * cf_scalar  # [N, 1]
                
                # 3.3 cd-cf 交互项（高阶效应）
                cd_cf_interaction = tf.convert_to_tensor([[cd_scalar * cf_scalar]], dtype=tf.float32)
                cd_cf_interaction = tf.tile(cd_cf_interaction, [tf.shape(pos)[0], 1])  # [N, 1]
                
                fluid_feats_list.extend([
                    cd_velocity_coupling,   # [N, 1]
                    cf_diffusion_coupling,  # [N, 1]
                    cd_cf_interaction       # [N, 1]
                ])
        ...
        # 后续层: 带残差连接的特征细化 + FiLM 调制（可选）
        shared_ans_convs = [processed_feats]
        for i, (conv, dense) in enumerate(zip(self.shared_convs, self.shared_denses)):
            ...
            # FiLM 调制（可选）
            if self.cd_cf_as_input and self.use_cd_cf_film and global_cd_cf_embedding is not None:
                film_layer = self.cd_cf_film_layers[i]
                gamma = film_layer['gamma_net'](global_cd_cf_embedding)  # [1, ch]
                beta = film_layer['beta_net'](global_cd_cf_embedding)    # [1, ch]
                
                # FiLM: h' = γ ⊙ h + β
                ans = ans * (1.0 + gamma) + beta
            
            # 残差连接（如果维度匹配）
            residual = shared_ans_convs[-1] if ans_dense.shape[-1] == shared_ans_convs[-1].shape[-1] else 0
            shared_ans_convs.append(ans + residual)
    ...
    def compute_delta_vf_from_shared(self, 
        ...
        def compute_vf_changes():
            ...
            def create_branch_fn(num_phases):
                ...
                def branch_fn():
                    # 列表索引：num_phases=2 -> index=0, num_phases=3 -> index=1
                    index = num_phases - 2
                    vf_conv = self.vf_conv_heads[index]
                    vf_dense = self.vf_dense_heads[index]
    ...
    def init(self, **kwargs) -> None:
        ...
        然后强制构建所有可能相数的 VF 预测头，确保权重完整初始化。
        这对于 .h5 格式的权重加载是必需的。
        """
        # ========== 定义虚拟初始化场景 ==========
        ...
        # ========== 执行初始化前向传播 ==========
        print(f"[Model V5] Initializing network weights with {init_num_phases}-phase scenario...")
        _ = self.__call__(
            (pos, vel, phase_fractions, box, box_feats),
            current_num_phases=tf.constant(init_num_phases, dtype=tf.int32),
            phase_densities=tf.constant(init_densities, dtype=tf.float32),
            cd=cd, 
            cf=cf
        )

        # ========== 强制构建所有 VF 预测头（关键修复！）==========
        # 这是为了确保所有预测头的权重都被创建，以便 .h5 文件能够成功加载
        # 对于 checkpoint 格式，这一步是可选的，但不会有负面影响
        if self.max_num_phases > 1 and len(self.vf_conv_heads) > 0:
            print(f"[Model V5] Building all VF prediction heads for phases 2 to {self.max_num_phases}...")
            filter_extent = tf.constant(self.filter_extent)
            dummy_shared_features = tf.zeros((1, self.shared_feature_channels[-1]), dtype=tf.float32)
            dummy_pos = tf.zeros((1, 3), dtype=tf.float32)
            
            for i, (conv_head, dense_head) in enumerate(zip(self.vf_conv_heads, self.vf_dense_heads)):
                num_phases = i + 2  # 因为从2相开始
                # 强制构建卷积层的权重
                _ = conv_head(dummy_shared_features, dummy_pos, dummy_pos, filter_extent)
                # 强制构建全连接层的权重
                _ = dense_head(dummy_shared_features)
                print(f"  YES!!! Built VF head for {num_phases} phases (output_dim={self.vf_output_dims[i]})")
            
            print(f"[Model V5] All {len(self.vf_conv_heads)} VF prediction heads successfully built.\n")
        
        # ========== 打印模型信息 ==========
        print(f"\n{'='*80}")
        print(f"[Model V5] Initialization Complete")
        print(f"{'='*80}")
        ...
        if self.cd_cf_as_input:
            print(f"\n  [V5 Conditional Components Initialized]")
            print(f"    - cd/cf Encoder       : Yes")
            print(f"    - Cross Attention     : {'Yes' if self.use_cd_cf_attention else 'No'}")
            print(f"    - FiLM Layers         : {'Yes' if self.use_cd_cf_film else 'No'} ({len(self.cd_cf_film_layers) if self.use_cd_cf_film else 0} layers)")
            print(f"    - Physics Features    : {'Yes' if self.use_cd_cf_physics else 'No'}")
        print(f"{'='*80}\n")
