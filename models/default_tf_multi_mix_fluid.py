...
class VolumeConservationLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, num_phases):
        super(VolumeConservationLayer, self).__init__(
            name="VolumeConservation")
...
class MultiPhaseParticleNetwork(tf.keras.Model):
    ...
    def __init__(self,
                 ...
                 num_phases=2,  # 流体相数量
                 cd_cf_as_input=True,
                 cd_cf_embedding_dim=16):  # 是否将cd/cf作为输入
        super().__init__(name=type(self).__name__)
        # 增加输出通道以处理每个相的修正
        self.layer_channels = [32, 64, 64, 3 +
                               num_phases - 1]  # 位置修正 + 体积分数修正(n-1)相
        ...
        self.cd_cf_embedding_dim = cd_cf_embedding_dim  # 保存嵌入维度
        self.initial_phase_volumes = None

        debug_print(f"particle_radius: {particle_radius}")
        debug_print(f"filter_extent: {self.filter_extent}")

        # 如果使用cd/cf嵌入，添加映射层
        if self.cd_cf_as_input and self.cd_cf_embedding_dim > 0:
            self.cd_embedding = tf.keras.layers.Dense(self.cd_cf_embedding_dim,
                                                      activation='tanh',
                                                      name='cd_embedding')
            self.cf_embedding = tf.keras.layers.Dense(self.cd_cf_embedding_dim,
                                                      activation='tanh',
                                                      name='cf_embedding')
    ...
    def compute_new_phase_fractions(self, old_fractions, correction):
        """计算更新的体积分数，确保整体体积守恒
        ...
        new_fractions = old_fractions + correction
        ...
        sum_per_particle = tf.reduce_sum(
            conserved_fractions, axis=-1, keepdims=True)
    ...
    def compute_correction(self,
        ...
        if (phase_fractions is not None):
            ...
            # 2. 添加体积梯度特征 - 在每个粒子位置计算体积分数的空间变化
            # 这需要在训练数据预处理中实现
            # if 'phase_gradients' in batch:
            #     fluid_feats.append(batch['phase_gradients'])

            # 3. 添加体积分数的非线性变换，增强表达能力
            # 例如使用平方和指数变换
            # fluid_feats.append(phase_fractions**2)  # 平方特征
            fluid_feats.append(tf.exp(phase_fractions) - 1.0)  # 指数特征
        ...
        if self.cd_cf_as_input:
            ...
            debug_print("cd:", cd_scalar)
            debug_print("cf:", cf_scalar)

            if self.cd_cf_embedding_dim > 0:
                # 将标量映射到高维嵌入空间
                # 先创建批次维度为1的输入
                cd_input = tf.reshape(cd_scalar, [1, 1])
                cf_input = tf.reshape(cf_scalar, [1, 1])

                # 应用嵌入层
                cd_embed = self.cd_embedding(cd_input)  # [1, embed_dim]
                cf_embed = self.cf_embedding(cf_input)  # [1, embed_dim]

                # 扩展到所有粒子
                batch_size = tf.shape(pos)[0]
                # [batch_size, embed_dim]
                cd_embed = tf.tile(cd_embed, [batch_size, 1])
                # [batch_size, embed_dim]
                cf_embed = tf.tile(cf_embed, [batch_size, 1])

                # 添加到特征
                fluid_feats.extend([cd_embed, cf_embed])
            else:
                # 原来的方法：直接使用标量
        ...
        # 缩放以更好地匹配输出分布
        raw_correction = self.ans_convs[-1]

        # 分离位置和体积分数修正，使用不同的缩放因子
        pos_correction = (1.0 / 128) * raw_correction[:, :3]  # 位置使用较小的缩放

        # 如果是多相流体，处理体积分数修正
        if self.num_phases > 1 and phase_fractions is not None:
            # vol_frac_correction = (1.0 / 32) * raw_correction[:, 3:]
            vol_frac_correction = raw_correction[:, 3:]

            # 应用体积守恒约束
            if self.volume_conservation is not None:
                vol_frac_correction = self.volume_conservation(
                    vol_frac_correction)
            ...
            # 合并为完整的修正向量
            correction = tf.concat(
                [pos_correction, vol_frac_correction], axis=-1)
            return correction
        else:
            self.pos_correction = pos_correction
    ...
    def set_initial_phase_volumes(self, phase_fractions):
        ...
        if self.num_phases > 1 and phase_fractions is not None:
            ...
            if self.initial_phase_volumes is None:
                ...
                # 如果已经初始化，则更新变量
                self.initial_phase_volumes.assign(
                    tf.reduce_sum(phase_fractions, axis=0))

            # 使用print而非tf.print，避免图模式问题
            print("Initial phase volumes set:",
                  self.initial_phase_volumes.numpy())
    ...
    def normalize_phase_fractions(self, fractions):
        ...
        # 确保每个粒子的体积分数总和不超过1
        sum_per_particle = tf.reduce_sum(
            fractions_conserved, axis=-1, keepdims=True)
    ...
    def call(self, inputs, fixed_radius_search_hash_table=None, cd=0.5, cf=0.5):
        ...
        if phase_fractions is not None and vol_frac_correction is not None:
            phase_fractions_corrected = self.compute_new_phase_fractions(
                phase_fractions, vol_frac_correction)
    ...
    def init(self, feats_shape=None):
        ...
        if self.num_phases > 1:
            ...
            phase_fractions = np.zeros(
                shape=(1, self.num_phases-1), dtype=np.float32)
            # 初始设置第一相的分数为1
