...
class MultiPhaseParticleNetwork(tf.keras.Model):
    ...
    def __init__(self,
                 ...
                 vf_contact_guard: bool = True,
                 vf_support_epsilon: float = 1e-4,
                 ) -> None:
        ...
        self.vf_contact_guard = vf_contact_guard
        self.vf_support_epsilon = vf_support_epsilon
    ...
    def _compute_neighbor_phase_mean(self,
                                     phase_values: tf.Tensor,
                                     neighbor_index: tf.Tensor,
                                     neighbor_row_splits: tf.Tensor) -> tf.Tensor:
        """
        聚合每个粒子邻域内的逐相平均 VF。

        这里使用 current_vf 的 0-1 物理占比，而不是 vf_scaled，
        因为 contact guard 关心的是“该相在当前粒子或邻域中是否真实存在”。

        邻居图由调用方显式传入，应与当前 VF 分支实际使用的空间邻域一致。
        若某粒子当前没有 fluid 邻居，则回退为它自己的当前 VF，
        这样后续的 contact guard 会自然得到“无接触 -> 不改相”。
        """
        num_particles = tf.shape(phase_values)[0]

        segment_ids = tf.ragged.row_splits_to_segment_ids(neighbor_row_splits)
        neighbor_phase_values = tf.gather(phase_values, neighbor_index)  # [E, P]

        phase_sum = tf.math.unsorted_segment_sum(
            neighbor_phase_values,
            segment_ids,
            num_particles,
        )
        neighbor_count = tf.math.unsorted_segment_sum(
            tf.ones_like(segment_ids, dtype=phase_values.dtype),
            segment_ids,
            num_particles,
        )

        safe_neighbor_count = tf.maximum(neighbor_count, 1.0)
        neighbor_phase_mean = phase_sum / safe_neighbor_count[:, None]
        has_neighbors = neighbor_count > 0
        return tf.where(has_neighbors[:, None], neighbor_phase_mean, phase_values)
    ...
    def _predict_next_vf(self,
        ...
        当 vf_contact_guard=True 时，额外施加两个物理护栏：
          1. 若某相既不在当前粒子中，也不在邻域中，则该相本步不能凭空出现。
          2. 若当前粒子与邻域的相分布几乎一致，则本步保持当前 VF，不做无接触均化。
        """
        filter_extent = tf.constant(self.filter_extent)

        # 空间卷积：让每个粒子在新位置处感知邻域 VF 状态
        vf_spatial = self.vf_context_conv(vf_branch_feats, pos, pos, filter_extent)  # [N, C]
        vf_neighbor_index = self.vf_context_conv.nns.neighbors_index
        vf_neighbor_row_splits = self.vf_context_conv.nns.neighbors_row_splits
        vf_spatial_expanded = tf.tile(
            tf.expand_dims(vf_spatial, axis=1), [1, num_phases, 1]
        )  # [N, num_phases, C]

        if self.vf_contact_guard:
            # 复用 vf_context_conv 刚构建的 pos_final 邻居图，避免与位置分支缓存邻居图不一致。
            neighbor_phase_mean = self._compute_neighbor_phase_mean(
                current_vf, vf_neighbor_index, vf_neighbor_row_splits
            )  # [N, P]
            phase_contact_strength = tf.clip_by_value(
                0.5 * tf.reduce_sum(tf.abs(neighbor_phase_mean - current_vf), axis=-1, keepdims=True),
                0.0,
                1.0,
            )
            phase_support_mask = tf.logical_or(
                current_vf > self.vf_support_epsilon,
                neighbor_phase_mean > self.vf_support_epsilon,
            )
        else:
            neighbor_phase_mean = current_vf
            phase_contact_strength = tf.ones_like(current_vf[:, :1])
            phase_support_mask = tf.ones_like(current_vf, dtype=tf.bool)

        # 设计意图：
        #   vf_spatial_expanded 提供当前粒子在新位置处的邻域 VF 上下文（对该粒子的所有相共享）
        #   vf_only_features   提供当前被更新相的逐相身份/占比信号
        # contact guard 用 neighbor_phase_mean 决定“能不能变、该不该变”，
        # 而不是让 MLP 直接靠它改输入维度，从而保持旧 checkpoint 的 VF 头形状兼容。
        ...
        masked_delta_logits = tf.where(
            phase_support_mask,
            delta_logits,
            tf.fill(tf.shape(delta_logits), tf.constant(-1e9, dtype=delta_logits.dtype)),
        )
        ...
        if self.vf_residual:
            ...
            vf_logits = tf.math.log(current_vf + 1e-8) + self.alpha * masked_delta_logits
        else:
            # 直接预测：网络输出绝对 VF logits，无恒等先验
            # 网络必须每步显式预测 VF 分布，不能通过 delta→0 敷衍
            vf_logits = masked_delta_logits

        predicted_vf = tf.nn.softmax(vf_logits, axis=-1)

        if not self.vf_contact_guard:
            return predicted_vf

        return (1.0 - phase_contact_strength) * current_vf + phase_contact_strength * predicted_vf
